...
public class VoteRepository implements IVoteRepository {
	...
	//private static String SELECT_TOP = "select *, count(*) as count from Vote group by article_id order by count desc limit 0, ?";
	private IInjectionProvider injectionProvider;
	
	public VoteRepository(IInjectionProvider injectionProvider){
...
/*
	public List<VotePO> getTop(int count) throws Exception {
		...
		try{
			...
			ResultSet rs = stat.executeQuery();
			try{
				List<VotePO> ret = resultSet2PO(rs);
				rs.close();
				return ret;
			}finally{
				try{ rs.close(); }catch(Exception e){}
